...
def find_next_index(save_dir: Path, exts=('png', 'jpg', 'jpeg')) -> int:
    pattern = re.compile(r'^(\d+)\.(' + '|'.join(re.escape(e) for e in exts) + r')$', re.IGNORECASE)
    max_index = 0
...
def capture_once(url: str, timeout: float) -> bytes:
    """Capture one image from the ESP32-CAM using a session.

    Uses ``Connection: close`` and a (connect, read) timeout tuple to
    avoid keep-alive / stuck connections on the ESP32.
    """
    headers = {
        "Connection": "close",
    ...
    # make sure the connection is closed promptly
    try:
        resp.close()
    except Exception:
        pass
...
def main() -> int:
    ...
    parser.add_argument('--url', '-u', default="http://10.165.107.241/capture", help='ESP32-CAM capture URL')
    parser.add_argument('--interval', '-i', default='5', help="Capture interval, e.g. '5', '0.5', '500ms' (ms suffix)")
    parser.add_argument('--total', '-n', type=int, default=100, help='Total images to capture')
    parser.add_argument('--save', '-d', default='dataset/train', help='Save directory')
    parser.add_argument('--format', '-f', default='png', choices=['png', 'jpg', 'jpeg'], help='Image extension')
    parser.add_argument('--timeout', type=float, default=10.0, help='HTTP timeout (s)')
    parser.add_argument('--max-retries', type=int, default=2, help='Max retries on capture failure')
    parser.add_argument('--retry-delay', type=float, default=2.0, help='Delay between retries (s)')
    parser.add_argument('--skip-failed', action='store_true', help='Skip failed captures and continue')
    parser.add_argument('--start-index', type=int, default=None, help='Optional start index override')
    ...
    while captured < args.total and not STOP:
        ...
        while attempts <= max_retries and not success and not STOP:
            try:
                ...
                filename = f"{next_index}.{image_format}"
        ...
        # maintain periodic schedule (reduce drift)
